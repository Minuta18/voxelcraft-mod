    ...
    for currX = startX, endX do
        for currY = startY, endY do
            for currZ = startZ, endZ do
				if get_block(currX, currY, currZ) ~= 0 and not is_replaceable_at(currX, currY, currZ) then
					set_block(currX, currY, currZ, id, get_block_states(currX, currY, currZ))
				elseif is_replaceable_at(currX, currY, currZ) then
					set_block(currX, currY, currZ, 0, get_block_states(currX, currY, currZ))
				end
            end
        end
    end
